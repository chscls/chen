/**  
* @Project: xbframe
* @Title: WebUploader.java
* @Package com.jeecms.common.upload
* @Description: TODO
* @author Administrator
* @date 2016年7月12日 上午12:36:51
* @Copyright: 2016
* @version V3.0  
*/
...
public class FileWebUploader {
	 ...
	 private final static Logger log = LoggerFactory.getLogger(FileWebUploader.class);
	     ...
	     * @param chunks    分片总数
	     * @param md5       文件签名
	     * @param path      待合并的临时文件位置
	     * @param outFilePath 合并后的文件路径
	     * @return
	     */
	    public String chunksMerge(String folder, String ext, int chunks, String md5, String path,String outFilePath){
	        ...
	        if(chunks == this.getChunksNum(path + "/" + folder)){
	        	File[] chunkFiles=this.getChunks(path + "/" +folder);
            	if(chunkFiles==null){
            		return null;
            	}
	            //同步指定合并的对象
	            ...
	            try{
	                ...
	                List<File> files = new ArrayList<File>(Arrays.asList(chunkFiles));
	                if(chunks == files.size()){
	                    ...
	                    if(outFilePath==null){
	                    	outputFile= new File(path + "/" + this.randomFileName(ext));
	                    }else{
	                    	outputFile=new File(outFilePath);
	                    	UploadUtils.checkDirAndCreate(outputFile.getParentFile());
	                    	outputFile = UploadUtils.getUniqueFile(outputFile);
	                    }
	     ...
	     * @return
	     */
	    public File getReadySpace(FileInfo info, String path,String fileName){
	     ...
	     * @throws
	     */
	    public String getFileNameMd5(FileInfo info){
	    	String filename=info.getUserId()+""+info.getName()
	    			+""+info.getLastModifiedDate()+""+info.getSize();
	    	return this.md5(filename);
	    }
}
